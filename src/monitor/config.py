...
# def ensure_app_yaml_exists():
#     """
#     Ensure that ~/.config/monitor/app.yaml exists.
#     If not, copy the default app.yaml from the package directory (next to this config.py)
#     into ~/.config/monitor/app.yaml, using permission mode 0600 for security.
#     Log success or handle exceptions appropriately.
#     """
#     user_config_dir = os.path.expanduser('~/.config/monitor')
#     user_app_yaml = os.path.join(user_config_dir, 'app.yaml')
#     # Source app.yaml (shipped with the package, in the same dir as config.py)
#     package_app_yaml = os.path.join(os.path.dirname(__file__), 'app.yaml')
#     if not os.path.exists(user_app_yaml):
#         try:
#             os.makedirs(user_config_dir, exist_ok=True)
#             with open(package_app_yaml, 'rb') as src, os.fdopen(os.open(user_app_yaml, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), 'wb') as dst:
#                 shutil.copyfileobj(src, dst)
#             logger.info(f"Copied default app.yaml to {user_app_yaml} with permissions 0600.")
#         except FileExistsError:
#             logger.info(f"app.yaml already exists at {user_app_yaml}.")
#         except Exception as e:
#             logger.error(f"Failed to copy {package_app_yaml} to {user_app_yaml}: {e}", exc_info=True)
#             # If user config cannot be created and does not exist, exit for safety.
#             if not os.path.exists(user_app_yaml):
#                 print(f"CRITICAL: Could not create or copy {user_app_yaml}. Exiting.")
#                 sys.exit(1)
#     return user_app_yaml

# def load_yaml_config(file_path=ensure_app_yaml_exists()):
#     # Always use the user's ~/.config/monitor/app.yaml
#     yaml_path = file_path
#     try:
#         with open(yaml_path) as f:
#             config = yaml.safe_load(f)
...
# # # --- CONFIGURATION FILE LOCATION AND CREATION ---
...
# # APP_YAML_PATH = ensure_app_yaml_exists()
...
# # public_commands_path_cfg = yaml_config.get("PUBLIC_COMMANDS_PATH", "~/.config/monitor/public_commands.json")
# # PUBLIC_COMMANDS_PATH = os.path.expanduser(public_commands_path_cfg)
...
# # preference_prompt_file = os.path.expanduser(preference_file_path)
