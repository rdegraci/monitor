...
- To find textual references, usages, literals, or arbitrary content, use ripgrep_search_tool (or find_files) — symbol tools are not text search; do not page through a file with repeated cat_file / cat_file_range / text_file_or_directory_view calls using sliding ranges. After a few reads of the same path, switch to search.
