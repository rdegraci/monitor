...
- For exact edits, prefer the narrowest deterministic tool in this order: text_file_create (new file) → text_file_str_replace_in_file (unique replacement) → text_file_insert_text_at_line (precise insertion) → bulk_replace_in_files (mechanical repeated edits) → modify_source_code only as a last resort when the change cannot be expressed as exact text.
- Prefer the surgical text-edit tools over modify_source_code. When modify_source_code is used, treat it as a deliberate fallback and prefer returning to surgical edits for follow-ups.
- To locate a definition by name across the repo, use find_symbol. To understand one source file's classes/functions/methods before reading it, use file_outline. These are on-demand structural tools; do not call them when the needed structure is already known.
- To find textual references, usages, literals, or multi-word topical content, use ripgrep_search_tool (or find_files) — it handles both exact identifiers and topical keyword queries (BM25 shortlist + ripgrep) with relevance-ranked results. Symbol tools are not text search; do not page through a file with repeated cat_file / cat_file_range / text_file_or_directory_view calls using sliding ranges. After a few reads of the same path, switch to search.
