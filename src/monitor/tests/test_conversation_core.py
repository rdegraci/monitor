...
class TestConversation(unittest.TestCase):
    @patch('core.conversation.prepare_query_context')
    @patch('core.conversation.get_llm_initial_completion')
    @patch('core.conversation.update_token_usage')
    @patch('core.conversation.process_response_by_type')
    @patch('core.conversation.determine_response_type', return_value='direct')
    ...
    @patch('core.conversation.config', new=MagicMock(MAX_TOKEN_COUNT=1048576))
    def test_handle_token_limit(self):
        self.assertEqual(conversation.handle_token_limit(42, 12), 1048576)

    @patch('core.conversation.prepend_memory_to_history')
    @patch('core.conversation.append_conversation_history')
    def test_prepare_query_context(self, mock_append, mock_prepend):
        # The function just calls the two helpers and passes params
        with patch('core.conversation.config', new=MagicMock()):
            with patch('core.conversation.SYSTEM_PROMPT', 'PROMPT'):
                conversation.prepare_query_context('foo')
                mock_prepend.assert_called_once()
                mock_append.assert_called_once()

    @patch('core.conversation.clear_screen')
    def test_conversation_history_command_empty(self, mock_clear):
        with patch('core.conversation.config', new=MagicMock(CONVERSATION_HISTORY=[])):
    ...
    def test_conversation_history_command_pagination(self, mock_input, mock_clear):
        ...
        with patch('core.conversation.config', new=MagicMock(CONVERSATION_HISTORY=fake_history)):
            with patch('builtins.print') as mock_print:
