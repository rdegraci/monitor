...
## Purpose
Treat this document as the authoritative operating manual. When reasoning about the repository, summarizing it, choosing files, or making changes, follow these rules exactly so results stay deterministic, traceable, and easy for tools to verify. This source is packaged as `instructions.md`, and the app reads the runtime copy from `appdir/monitor/instructions.md` at runtime.
...
When inferring architecture, treat path-based layer mapping as the first and strongest signal. Use it to determine which code owns the behavior and which files are relevant.
...
When choosing files to inspect or modify, prefer the files whose names match the behavior you need to change.
...
When tracing dependencies, follow the ownership direction implied by the layer: start from orchestration or entry points, then follow collaborators downward.
...
- Keep repository-specific working instructions separate when possible.
...
When modifying code, make durable changes that match the repository's architecture instead of patching symptoms.
