...
def test_new_transcript_events_restore_tail_follow() -> None:
    ...
    assert app._transcript_viewport.follow_tail is False
...
def test_background_completion_updates_status_and_task_id() -> None:
    ...
    event = type("BackgroundCompletionEvent", (), {"success": True})()

    app._handle_event(event)

    assert app.status_text == "completed"
    assert app.active_task_id == "task-1"
    assert len(coordinator.completed_events) == 1
