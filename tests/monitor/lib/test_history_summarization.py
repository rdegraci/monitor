...
def conversation_and_config(sample_conversation, config):
    ...
    config.TOTAL_TOKEN_COUNT = recount_conversation_tokens(conversation, count_message_tokens_always_10)
    return conversation, config

@pytest.mark.parametrize("trigger,modify_func,expected_trigger", [
    (
        "token_threshold",
    ...
    (
        "memory_limit_mb",
        lambda convo, cfg: _simulate_memory_trigger(cfg, convo, cfg.SUMMARIZATION_CONFIG['triggers']['memory_limit_mb']),
        True
    ),
    (
        "token_threshold (negative)",
        lambda convo, cfg: _set_strictly_below_token_threshold(cfg, convo, cfg.SUMMARIZATION_CONFIG, cfg.MAX_TOKEN_COUNT, logger=cfg.__dict__.get("_pytest_logger")),
        False
    ),
    (
        "conversation_size (negative)",
        lambda convo, cfg: _set_strictly_below_conversation_size(cfg, convo, cfg.CONVERSATION_MAX_SIZE, logger=cfg.__dict__.get("_pytest_logger")),
        False
    ),
    (
        "time_limit_seconds (negative)",
        lambda convo, cfg: _set_strictly_below_time_trigger(cfg, convo, cfg.SUMMARIZATION_CONFIG['triggers']['time_limit_seconds']),
        False
    ),
    (
...
def test_summarization_triggers_and_reset(
    ...
    conversation_and_config, logger, monkeypatch
):
    ...
    if expected_trigger:
        assert should_summarize, f"Expected summarization for trigger '{trigger}', but it was not triggered"
    else:
        assert not should_summarize, f"Did not expect summarization for negative case of '{trigger}'"
    ...
    if should_summarize:
        ...
        logger.info(f"After reset: conversation len={len(conversation)}, tokens={config.TOTAL_TOKEN_COUNT}")
        assert len(conversation) >= 2
        recounted = recount_conversation_tokens(conversation, count_message_tokens_always_10)
        assert config.TOTAL_TOKEN_COUNT == recounted
        summary_content = summary_resp.choices[0].message["content"]
        found_summary_content = any(summary_content in m.get("content", "") for m in conversation)
...
def _fill_tokens_to_threshold(config, conversation, summarization_config, max_token_count):
    ...
    while config.TOTAL_TOKEN_COUNT < threshold_tokens + 1:
        msg = {"role": "user", "content": f"Payload token_filler {config.TOTAL_TOKEN_COUNT}"}
...
def _set_strictly_below_token_threshold(config, conversation, summarization_config, max_token_count, logger=None):
    ...
    if tokens_remaining > 0 and len(user_msg_objs) < max_user_msgs:
        ...
        if tokens_remaining == 10:
            user_msg_objs.append({"role": "user", "content": f"Msg {len(user_msg_objs)}"})
            user_msg_tokens += 10
            total_so_far = base_token_count + user_msg_tokens
    ...
    assert config.TOTAL_TOKEN_COUNT <= threshold_tokens - SAFE_MARGIN and len(conversation) <= conversation_max_size - 2, (
...
def _set_strictly_below_conversation_size(config, conversation, conversation_max_size, logger=None):
    """
    ...
    config.TOTAL_TOKEN_COUNT = recount_conversation_tokens(conversation, count_message_tokens_always_10)
    if logger: logger.info(f"[NEGATIVE/ConvSize] conv_max={conversation_max_size}, margin={SAFE_MARGIN}, set len={len(conversation)}")
    ...
    if logger: logger.info(f"Final negative-convsize setup: length={len(conversation)} (should be <= {conversation_max_size-SAFE_MARGIN}), token_count={config.TOTAL_TOKEN_COUNT}")
    config.last_summary_time = 100000.0
    if hasattr(config, '_fake_now'):
        config._fake_now = 100000.0
    if logger: logger.info(f"Set negative-case time-neutral: last_summary_time={config.last_summary_time}, _fake_now={getattr(config, '_fake_now', None)}")
...
def _set_strictly_below_memory_trigger(config, conversation, memory_limit_mb, logger=None):
    """
    ...
    """
    SAFE_MARGIN_MB = 10
    target_limit = int((memory_limit_mb * 1024 * 1024)) - (SAFE_MARGIN_MB * 1024 * 1024)
    ...
    config.TOTAL_TOKEN_COUNT = recount_conversation_tokens(conversation, count_message_tokens_always_10)
    if logger: logger.info(f"[NEGATIVE/Memory] limit={memory_limit_mb} MB, safe_margin={SAFE_MARGIN_MB} MB, simulated_bytes={target_limit}")
    assert target_limit <= int((memory_limit_mb * 1024 * 1024)) - (SAFE_MARGIN_MB * 1024 * 1024), \
    ...
    if logger: logger.info(f"Final negative-memory setup: __sizeof__={target_limit} (should be <= {int((memory_limit_mb * 1024 * 1024)) - (SAFE_MARGIN_MB * 1024 * 1024)}), conv_len={len(conversation)}, tokens={config.TOTAL_TOKEN_COUNT}")
    config.last_summary_time = 100000.0
    if hasattr(config, '_fake_now'):
        config._fake_now = 100000.0
    if logger: logger.info(f"Set negative-case time-neutral: last_summary_time={config.last_summary_time}, _fake_now={getattr(config, '_fake_now', None)}")
...
def test_prompt_count_stale_after_summarization(logger, config, monkeypatch):
    ...
    conversation = [
        {"role": "system", "content": "System prompt."}
    ] + [{"role": "user", "content": f"Message {i}"} for i in range(config.CONVERSATION_MAX_SIZE)]
    config.TOTAL_TOKEN_COUNT = recount_conversation_tokens(conversation, count_message_tokens_always_10)
    ...
    assert limits.get("should_summarize", False), "Test conversation should trigger summarization reset"
    ...
    assert old_count != new_count, "Summarization should change (usually shorten) the conversation length"
