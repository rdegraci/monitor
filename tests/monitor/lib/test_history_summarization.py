...
def test_prompt_count_stale_after_summarization(logger, config, monkeypatch):
    ...
    # Format note: after summarization/compaction fires, the H indicator uses
    # a compaction-count prefix: "H:(N) <count>".
    assert (
        f"{old_count}" in display_with_stale_count
    ), "Display must show the old conversation count before summarization"
    assert (
        f"{new_count}" in display_with_live_count
