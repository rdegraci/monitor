...
- Confirm planned configuration bootstrap behavior for `monitor_oop` is implemented and verified: preserve existing `appdirs.user_config_dir("monitor")/config.yaml` files, seed only missing files from `src/monitor_oop/config.yaml.example` into the user config directory when needed, load `config.yaml` from the user config directory with fallback to `~/.config/monitor/`, and load `.env` with `find_dotenv(usecwd=True)` plus user config fallbacks.
